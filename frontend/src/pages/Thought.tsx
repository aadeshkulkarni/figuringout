...
export const Thought = ()=>{
    ...
    const {fetchedThoughts, setFetchedThoughts} = useState<{id : string, title:string , content : string} | null>(null)
    return (
        <div>
                ...
                <div>
                    ...
                    <div>
                        <button onChange = {async ()=>{
                            const res = axios.post(`${BACKEND_URL}/api/v1/thought/`, {
                                content,
                                title
                            })
                    ...
                    <div>
                        <button onClick = {async ()=>{
                            const thoughts = axios.get(`${BACKEND_URL}/api/v1/thought/bulk`)
                            setFetchedThoughts(thoughts.data)
                        }}>Show all thoughts</button>
                    </div>
                    <div>
                        <button onClick = {async ()=>{
                            const res = axios.delete(`${BACKEND_URL}/api/v1/thought/paste_id_here`)
                            
                        }}>Delete Thought(Paste id in axios url to delete particular)</button>
                    </div>
                    <div>
                        <button onClick = {async ()=>{
                            const res = axios.put(`${BACKEND_URL}/api/v1/thought/`, {
                                content,
                                title
                            })
