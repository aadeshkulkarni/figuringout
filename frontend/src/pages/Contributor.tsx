        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 ">
          {contributors.map((contributor: any) => (
            <a
              ...
              className="bg-white rounded-lg p-4 flex flex-col items-center justify-center transform transition-transform duration-300 hover:scale-105 shadow-2xl dark:bg-gray-950 dark:text-white dark: border "
              style={{ boxShadow: '0 0 20px -10px rgba(0, 0, 0, 0.4)' }}
            >
              <div className="bg-gray-200 w-20 h-20 rounded-full flex items-center justify-center mb-4 overflow-hidden dark:bg-gray-900">
                <img src={contributor?.avatar_url} alt={contributor?.login} className="w-full h-full object-cover" />
              </div>

              <div className="text-center">
                <div className="inline-block bg-gray-200 dark:bg-white rounded-full">
                  <GitHubIcon />
                </div>

                <h2 className="text-lg font-medium">{contributor.login}</h2>
                <p className="text-gray-600 dark:text-gray-300">{contributor?.contributions} contributions</p>
              </div>
            </a>
          ))}
        </div>
