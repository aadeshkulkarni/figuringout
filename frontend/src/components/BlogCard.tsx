...
const BlogCard: React.FC<BlogCardProps> = ({ author, title, content, publishedDate, id, fullWidth, tagsOnPost }) => {
  ...
  return (
    <Link
      to={`/blog/${id}`}
      className={`snap-center min-h-screen md:min-h-min md:h-min blog-card md:px-4 md:py-8 ${fullWidth ? 'w-full' : 'w-full md:w-3/6'} md:border-b md:border-main my-1 bg-opacity-10 shadow-b-sm flex flex-col justify-between md:grid md:grid-cols-12 md:gap-6 lg:gap-0`}
    >
